...
console.log("Hello World");
    ...
    function draw(pageX,pageY,radius) {
            ctx.beginPath();
            ctx.arc(pageX, pageY, radius, 0, 2 * Math.PI, false);
            //ctx.fillStyle = 'green';
            ctx.globalAlpha=1;
            ctx.strokeStyle = 'red';
            ctx.lineWidth = '8';
	...
	function touchStart(event) {
        ...
        draw_id = setInterval(function(){ 
            ...
            radius = radius+1;
            draw(pageX-left,pageY-top,radius); }, 10);
	}
    ...
    function touchEnd(event) {
            ...
            min_X = relativeX - radius;
            max_X = relativeX + radius;
            min_Y = relativeY - radius;
            max_Y = relativeY + radius;
            if(min_X<0){
                min_X=0;
            }
            if(min_Y<0){
            ...
            var formdata = {"img_size":{"width":temp.css("width"),"height":temp.css("height")}
                ...
                $.ajax({
                    ...
                    dataType: 'json',
                    url: 'http://35.227.183.188:5000/long_clickpost',
                    success: function (e) {
    ...
    function touch_mask(event){ 
        ...
        ctx.globalAlpha=0.5;
        ctx.fillStyle="white";
        ctx.arc(pageX-left, pageY-top, radius, 0, 2 * Math.PI, false);
        ctx.fill();
    }	
    ...
    /*function draw() {
            ...
            //ctx.stroke();
            ctx.fillStyle = "blue";
            ctx.arc(400, 300, g_radius, 0, 2 * Math.PI);
            ctx.fill();
            //ctx.lineWidth = '8';
            //ctx.stroke();
            ctx.closePath();
